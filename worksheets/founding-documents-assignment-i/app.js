...
const CHECK_RULES=[
  ...
  [1,1,/tax|taxes|taxing|taxation|levy|revenue|money|debt|fund|army|military|defen[sc]e|enforc|trade|commerce/],
  ...
  [2,1,/prohibit|prohibited|ban|banned|forbid|forbade|forbidden|no slavery|neither slavery|slavery.{0,15}(illegal|not allowed)|involuntary servitude/],
  ...
  [3,1,/same|equal|not inferior|full state|same power|same right|second.class|share.{0,20}(federal|national).{0,15}council|federal council|national council|voice.{0,15}(government|congress)|representation/],
...
async function post(action,data,saveReason=''){if(!configured())return false;const body=new URLSearchParams({action,saveReason,payload:JSON.stringify(data)});await fetch(CONFIG.appsScriptUrl,{method:'POST',mode:'no-cors',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});return true;}
...
function answerPasses(answer,rule){const text=String(answer||'').toLowerCase().replace(/[’']/g,"'").replace(/\s+/g,' ').trim();const words=text.match(/[a-z0-9]+/g)||[];if(!words.length)return false;const concepts=new Set();for(let i=2;i<rule.length;i++){const flags=rule[i].flags.includes('g')?rule[i].flags:`${rule[i].flags}g`;const matches=text.match(new RegExp(rule[i].source,flags))||[];matches.forEach(match=>concepts.add(match.toLowerCase().trim()));}if(concepts.size>=rule[1])return true;return words.length>=4;}
...
async function checkAnswers(){const missing=['studentName','period','email'].filter(id=>!$(`${id}`).value.trim());if(missing.length){missing.forEach(id=>$(`${id}`).classList.add('field-error'));status('Enter name, period, and school email before checking answers.',true);$(`${missing[0]}`).focus();return;}latestCheck={};questions.forEach((_,i)=>{const answer=$(`q${i+1}`).value.trim();const passed=answerPasses(answer,CHECK_RULES[i]);latestCheck[`q${i+1}`]=passed;showCheckResult(i,passed,!answer);});checkCount++;const data=localSave(false);const score=Object.values(latestCheck).filter(Boolean).length;status(`Check ${checkCount}: ${score} of ${questions.length} look good. Your current work is being saved.`);try{await post('save',data,'check');status(`Check ${checkCount}: ${score} of ${questions.length} look good. Updated work saved.`);}catch(e){status(`Check ${checkCount}: ${score} of ${questions.length} look good. Saved on this device; cloud save was unavailable.`,true);}}

render();
installPeriodOptions();
installCheckButton();
installTelemetry();
document.addEventListener('input',()=>{updateProgress();cloudDirty=true;clearTimeout(window.autoSaveTimer);window.autoSaveTimer=setTimeout(()=>localSave(false),600);clearTimeout(window.cloudAutoSaveTimer);window.cloudAutoSaveTimer=setTimeout(cloudAutoSave,45000);});
setInterval(cloudAutoSave,120000);
$('saveBtn').addEventListener('click',async()=>{const data=collect();if(answerCountIn(data)===0){status('Blank work was not saved. Use Retrieve Previous Work first.',true);return;}localSave();try{if(await post('save',data,'manual'))status('Saved on this device and to your cloud draft.');}catch(e){status('Saved on this device; cloud save was unavailable.',true);}});
$('retrieveBtn').addEventListener('click',async()=>{if(!$('email').value.trim()){status('Enter your school email first.',true);return;}const exact=localStorage.getItem(storageKey());let local=null;try{local=exact?JSON.parse(exact):null;}catch(e){}const rescued=bestLocalCopy();if(rescued&&answerCountIn(rescued)>answerCountIn(local))local=rescued;let cloud=null;try{cloud=await cloudRetrieve();}catch(e){}const localCount=answerCountIn(local);const cloudCount=answerCountIn(cloud);const best=cloudCount>localCount?cloud:local;if(best&&answerCountIn(best)>0){fill(best);localSave(false);status(cloudCount>localCount?`Recovered ${cloudCount} answers from the spreadsheet.`:`Recovered ${localCount} answers from this Chromebook. No shorter copy was allowed to replace them.`);}else{status('No saved copy was found. Check that the same Chromebook and exact school email are being used.',true);}});
$('assignmentForm').addEventListener('submit',async e=>{e.preventDefault();if(!validate()){$('submitStatus').textContent='Please complete every highlighted field.';return;}if(!$('honorCheck').checked)return;telemetryState.submittedAt=new Date().toISOString();finishQuestionTimer();const data=localSave(false);$('submitBtn').disabled=true;$('submitStatus').textContent='Submitting…';try{if(configured()){await post('submit',data,'submit');localStorage.setItem(`${storageKey()}:submitted`,data.savedAt);$('submitStatus').textContent='Submitted successfully. Your teacher now has your responses.';}else{$('submitStatus').textContent='Your work is complete and saved on this device. Your teacher must configure the submission URL before it can be sent.';}}catch(err){telemetryState.submittedAt='';$('submitStatus').textContent='Submission could not be sent. Your work remains saved on this device; try again.';$('submitBtn').disabled=false;}});
updateProgress();
